...
// --- SCHEDULING RULE: Helper function to validate appointment times ---
const isAppointmentTimeValid = (date, time) => {
    ...
    // The 'Z' at the end tells the parser the time is in UTC.
    ...
    // Check if the time is within the 09:00 - 17:00 range.
...
// In server.js, replace your entire setupDatabase function with this one:
async function setupDatabase() {
    ...
    try {
        ...
        // Define the availability variable FIRST...
...
app.post('/api/appointments', authMiddleware, async (req, res) => {
    ...
    // --- SCHEDULING RULE: Validate the requested date and time ---
...
app.post('/api/appointments/:id/reschedule', authMiddleware, async (req, res) => {
    ...
    // --- SCHEDULING RULE: Validate the new date and time ---
    if (!isAppointmentTimeValid(date, time)) {
...
app.put('/api/users/profile', authMiddleware, async (req, res) => {
    ...
    try {
        const client = await pool.connect();
        const currentUser = await client.query('SELECT * FROM users WHERE id = $1', [req.user.id]);
        if (currentUser.rows.length === 0) { client.release(); return res.status(404).json({ message: 'User not found.' });}
        const user = currentUser.rows[0];
        const updates = {
            name: name || user.name, phone: phone || user.phone, department: department || user.department,
            year_semester: req.user.role === 'student' ? (yearSemester || user.year_semester) : user.year_semester,
            designation: req.user.role === 'faculty' ? (designation || user.designation) : user.designation,
            office: req.user.role === 'faculty' ? (office || user.office) : user.office,
        };
        const result = await client.query( `UPDATE users SET name = $1, phone = $2, department = $3, year_semester = $4, designation = $5, office = $6 WHERE id = $7 RETURNING *`,
            [updates.name, updates.phone, updates.department, updates.year_semester, updates.designation, updates.office, req.user.id]
        );
        client.release();
...
// Save the server instance to a variable
const server = app.listen(PORT, async () => {
    console.log(`Server is running on http://localhost:${PORT}`);
    // await setupDatabase(); // Run once to set up, then comment out
});
...
const gracefulShutdown = () => {
    ...
    // Force close if it takes too long (e.g., 5 seconds)
    setTimeout(() => {
...
// Listen for termination signals (Ctrl+C, kill, nodemon restart)
